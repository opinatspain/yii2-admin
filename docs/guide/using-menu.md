...
```php
use opinatspain\admin\components\MenuHelper;
...
Return of `opinatspain\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `opinatspain\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `opinatspain\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
